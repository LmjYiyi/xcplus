...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath,String objectName) {
        ...
        if(!result){
            XueChengPlusExecption.cast("上传文件失败");
        }
        //入库文件信息
        MediaFiles mediaFiles = currentProxy.addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDto, bucket_mediafiles, objectName);
        if(mediaFiles==null){
            XueChengPlusExecption.cast("文件上传后保存信息失败");
        }
        //准备返回的对象
    ...
    public void addWaitingTask(MediaFiles mediaFiles){
        log.debug("Adding waiting task for media file: {}", mediaFiles.getFilename());
        ...
        if(mimeType.equals("video/x-msvideo")){//如果是avi视频写入待处理任务
            ...
            int insertResult = mediaProcessMapper.insert(mediaProcess);
            log.debug("Insert result: {}", insertResult);
        } else {
            log.debug("File is not an avi video, skipping task addition.");
        }
    }
