...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        ...
        String qrCode = null;
        try {
            qrcodeurl = String.format(qrcodeurl, payRecord.getPayNo());//拼接url
            qrCode = new QRCodeUtil().createQRCode(qrcodeurl, 200, 200);
        } catch (IOException e) {
            XueChengPlusExecption.cast("生成二维码出错");
        }
    ...
    public PayStatusDto queryPayResultFromAlipay(String payNo) {
        ...
        try {
            ...
            if (!response.isSuccess()) {
                XueChengPlusExecption.cast("请求支付查询查询失败");
            }
    ...
    public void saveAlipayStatus(PayStatusDto payStatusDto) {
        ...
        if (payRecord == null) {
            XueChengPlusExecption.cast("未找到支付记录");
        }
        XcOrders order = xcOrdersMapper.selectById(payRecord.getOrderId());
        if (order == null) {
            XueChengPlusExecption.cast("找不到相关联的订单");
        }
        ...
        if ("TRADE_SUCCESS".equals(tradeStatus)) {
            ...
            if (updateRecord <= 0) {
                XueChengPlusExecption.cast("更新支付交易表失败");
            }
            // 更新订单表
            ...
            if (updateOrder <= 0) {
                log.debug("更新订单表失败");
                XueChengPlusExecption.cast("更新订单表失败");
            }
        }
    ...
    public XcOrders saveOrders(String userId, AddOrderDto addOrderDto) {
        ...
        if (insert <= 0) {
            XueChengPlusExecption.cast("插入订单记录失败");
        }
        // 3. 插入订单明细表
        ...
        xcOrdersGoodsList.forEach(goods -> {
            ...
            if (insert1 <= 0) {
                XueChengPlusExecption.cast("插入订单明细失败");
            }
        });
        return order;
    }
    ...
    public XcPayRecord createPayRecord(XcOrders orders) {
        if (orders == null) {
            XueChengPlusExecption.cast("订单不存在");
        }
        if ("600002".equals(orders.getStatus())) {
            XueChengPlusExecption.cast("订单已支付");
        }
        ...
        if (insert <= 0) {
            XueChengPlusExecption.cast("插入支付交易记录失败");
        }
        return payRecord;
    }
