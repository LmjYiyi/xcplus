...
public class OrderController {
    ...
    public PayRecordDto generatePayCode(@RequestBody AddOrderDto addOrderDto) {
        ...
        if (user == null) {
            XueChengPlusExecption.cast("请登录后继续选课");
        }
    ...
    public void requestpay(String payNo, HttpServletResponse response) throws AlipayApiException, IOException {
        ...
        if (payRecord == null) {
            XueChengPlusExecption.cast("请重新点击支付获取二维码");
        }
        String status = payRecord.getStatus();
        if ("601002".equals(status)) {
            XueChengPlusExecption.cast("订单已支付，请勿重复支付");
        }
        AlipayClient alipayClient = new DefaultAlipayClient(AlipayConfig.URL, APP_ID, APP_PRIVATE_KEY, AlipayConfig.FORMAT, AlipayConfig.CHARSET, ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);
        //获得初始化的AlipayClient
        AlipayTradeWapPayRequest alipayRequest = new AlipayTradeWapPayRequest();//创建API对应的request
//        alipayRequest.setReturnUrl("http://domain.com/CallBack/return_url.jsp");
        alipayRequest.setNotifyUrl("http://i2kte2.natappfree.cc/orders/paynotify");//在公共参数中设置回跳和通知地址
